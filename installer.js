...
switch (process.platform) {
  ...
  case "darwin":
    deleteFolderRecursive(`${__dirname}/compilers/linux_64`);
    console.log("Downloading Darwin x64 Go binaries");
    https.get(
      "https://redirector.gvt1.com/edgedl/go/go1.9.2.darwin-amd64.pkg",
      function(response) {
        response.on("end", () => {
          console.log("Finished downloading, now decompressing...");
          targz.decompress(
            {
              src: `${__dirname}/compilers/darwin_64.tar.gz`,
              dest: `${__dirname}/compilers/`
            },
            function(err) {
              fs.rename(
                `${__dirname}/compilers/go`,
                `${__dirname}/compilers/darwin_64`,
                function(err) {
                  if (err) throw err;
                }
              );
              fs.unlink(`${__dirname}/compilers/darwin_64.tar.gz`);
              if (err) {
                console.log(err);
              } else {
                console.log("Decompressed!");
        ...
        response.pipe(
          fs.createWriteStream(`${__dirname}/compilers/darwin_64.tar.gz`)
        );
      }
    );
    break;
